<div x-show="openDeleteModal" x-transition:enter="transition ease-out duration-300"
    ...
    <div @click.away="openDeleteModal = false"
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            <button @click="openDeleteModal = false" class="text-gray-400 hover:text-gray-600 font-bold">&times;</button>
        </div>

        <div class="mb-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm">
            <p class="font-semibold">¿Estás seguro de eliminar a este estudiante?</p>
            <p class="mt-1">El registro de <span class="font-bold underline" x-text="deleteNombre"></span> dejará de estar activo como estudiante.</p>
        </div>

        <form :action="`/estudiantes/${deleteId}`" method="POST">
            @csrf
            @method('DELETE')
