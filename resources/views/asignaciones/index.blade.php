    ...
    <div class="space-y-6" x-data="{
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                <h3 class="font-bold text-gray-800 text-lg">Lista de Asignaciones Registradas</h3>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Grado</th>
                        ...
                        @forelse ($asignaciones as $asignacion)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-gray-500 font-medium">
                                    {{ $asignacion->id_asignacion }}</td>
                                <td class="px-6 py-4 whitespace-nowrap font-semibold text-gray-900">
                                    {{ $asignacion->docente->persona->nombres ?? 'N/A' }}
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-gray-700">
                                    <span
                                        ...
                                        {{ $asignacion->nivel->nivel ?? 'N/A' }} ({{ $asignacion->nivel->turno ?? '' }})
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-gray-700">
                                    {{ $asignacion->grado->curso ?? 'N/A' }} "{{ $asignacion->grado->paralelo ?? '' }}"
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-gray-700">
                                    <span
                                        class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-teal-50 text-teal-700">
                                        {{ $asignacion->materia->nombre_materia ?? 'N/A' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right space-x-3">
                                    <button
                                        @click="openEditModal = true; editId = '{{ $asignacion->id_asignacion }}'; editDocenteId = '{{ $asignacion->docente_id }}'; editNivelId = '{{ $asignacion->nivel_id }}'; editGradoId = '{{ $asignacion->grado_id }}'; editMateriaId = '{{ $asignacion->materia_id }}'"
                                        class="text-indigo-600 hover:text-indigo-900 font-medium">Editar</button>
                                    <button
                                        @click="openDeleteModal = true; deleteId = '{{ $asignacion->id_asignacion }}'; deleteNombre = 'Docente: {{ $asignacion->docente->persona->nombre ?? '' }} - Materia: {{ $asignacion->materia->nombre_materia ?? '' }}'"
                                        class="text-red-600 hover:text-red-900 font-medium">Eliminar</button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 text-center text-gray-500">No hay asignaciones
                                    registradas.</td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        @include('asignaciones.modals.edit')
        @include('asignaciones.modals.delete')
    </div>
</x-app-layout>
