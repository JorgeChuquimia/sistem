<div x-show="openEditModal" x-transition:enter="transition ease-out duration-300"
    ...
    <div @click.away="openEditModal = false"
        ...
        <form :action="`/docentes/${editId}`" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">RDA</label>
                <input type="text" name="rda" x-model="editRda" required
                    class="w-full rounded-xl border-gray-200 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Especialidad</label>
                <input type="text" name="especialidad" x-model="editEspecialidad" required
                    class="w-full rounded-xl border-gray-200 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Antigüedad</label>
                <input type="text" name="antiguedad" x-model="editAntiguedad" required
                    class="w-full rounded-xl border-gray-200 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
            </div>
