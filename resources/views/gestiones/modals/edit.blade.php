<div x-show="openEditModal" x-transition:enter="transition ease-out duration-300"
    ...
    <div @click.away="openEditModal = false"
        ...
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold text-gray-900">Editar Gestión</h3>
            <button @click="openEditModal = false" class="text-gray-400 hover:text-gray-600 font-bold">&times;</button>
        </div>

        <form :action="`/gestiones/${editId}`" method="POST" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Año / Gestión</label>
                <input type="text" name="gestion" x-model="editGestion" required class="w-full rounded-xl border-gray-200 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
            </div>
