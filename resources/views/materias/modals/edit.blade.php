<div x-show="openEditModal" x-transition:enter="transition ease-out duration-300"
    ...
    <div @click.away="openEditModal = false"
        ...
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold text-gray-900">Editar Materia</h3>
            <button @click="openEditModal = false" class="text-gray-400 hover:text-gray-600 font-bold">&times;</button>
        </div>

        <form :action="`/materias/${editId}`" method="POST" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Nombre de la Materia</label>
                <input type="text" name="nombre_materia" x-model="editNombreMateria" required
